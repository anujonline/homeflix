...
- Backend proxy: `backend/src/main/java/com/homeflix/controller/TmdbProxyController.java:13` proxies TMDB (`/api/trending`, `/{type}/popular`, `/movie/top_rated`, `/tv/top_rated`, `/{type}/{id:\d+}`, `/search?query=`, `/genres/*`) with Caffeine 6-hour cache (`backend/src/main/resources/application.yml:28` `maximumSize=500,expireAfterWrite=6h`); search endpoints bypass the cache and hit TMDB live. `TmdbService.java:32` injects Bearer token server-side.
