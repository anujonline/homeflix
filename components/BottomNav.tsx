...
import { Home, Film, Tv, TrendingUp, Library } from 'lucide-react';
    ...
    <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 dark:bg-[#020617]/95 backdrop-blur-xl border-t border-slate-200 dark:border-white/10">
      <div className="flex justify-around px-1 py-1.5 safe-area-pb">
        {items.map(it => {
          ...
          return (
            <button key={it.id} onClick={()=>onViewChange(it.id)} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-2xl min-w-[64px] ${active ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500'}`}>
